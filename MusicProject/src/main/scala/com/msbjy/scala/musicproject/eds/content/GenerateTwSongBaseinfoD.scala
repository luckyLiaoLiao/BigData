...
//      sparkSession对象一定会操作hive，配置config("hive.metastore.uris",hiveMetaStoreUris)可以在本地操作hive。
//      该配置项配置的是hive元数据的路径。enableHiveSupport()开启hive的支持才能操作hive数据
//      getOrCreate()获取出来sparkSession对象
//      如果不想配置config("hive.metastore.uris",hiveMetaStoreUris)，可以在src/main/resources中把hive-site.xml复制过来也可以
    ...
    sparkSession.table("TO_SONG_INFO_D")//加载这张表
      .withColumn("ALBUM",udfGetAlbumName(col("ALBUM")))//先读出这一列，清洗以后再存进去
...
//    创建一个临时视图，是session级别的，会随着会话的消失而消失
